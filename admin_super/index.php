...
// $stmt = $pdo->query("SELECT COUNT(*) FROM thesis_submissions");
// $totalThesis = $stmt->fetchColumn();

$stmt = $pdo->query("SELECT COUNT(DISTINCT student_id) FROM assignments");
...
// thesis proposal query in the dashboard
$stmt = $pdo->query("SELECT COUNT(*) FROM thesis_proposals");
$totalThesisSubmitted = $stmt->fetchColumn();


$stmt = $pdo->query("SELECT COUNT(*) FROM thesis_proposals WHERE status = 'pending'");
...
$stmt = $pdo->query("SELECT COUNT(*) FROM thesis_proposals WHERE status = 'rejected'");;
$totalThesisSubmittedRejected = $stmt->fetchColumn();



// thesis chapter One query in the dashboard
$stmt = $pdo->query("SELECT COUNT(*) FROM chapter_one");
$totalchapter_oneSubmitted = $stmt->fetchColumn();


$stmt = $pdo->query("SELECT COUNT(*) FROM chapter_one WHERE status = 'pending'");
$totalchapter_oneSubmittedPending = $stmt->fetchColumn();


$stmt = $pdo->query("SELECT COUNT(*) FROM chapter_one WHERE status = 'approved'");;
$totalchapter_oneSubmittedApproved = $stmt->fetchColumn();


$stmt = $pdo->query("SELECT COUNT(*) FROM chapter_one WHERE status = 'rejected'");;
$totalchapter_oneSubmittedRejected = $stmt->fetchColumn();



// thesis chapter two query in the dashboard
$stmt = $pdo->query("SELECT COUNT(*) FROM chapter_two");
$totalchapter_twoSubmitted = $stmt->fetchColumn();


$stmt = $pdo->query("SELECT COUNT(*) FROM chapter_two WHERE status = 'pending'");
$totalchapter_twoSubmittedPending = $stmt->fetchColumn();


$stmt = $pdo->query("SELECT COUNT(*) FROM chapter_two WHERE status = 'approved'");;
$totalchapter_twoSubmittedApproved = $stmt->fetchColumn();


$stmt = $pdo->query("SELECT COUNT(*) FROM chapter_two WHERE status = 'rejected'");;
...
$stmt = $pdo->query("SELECT COUNT(*) FROM chapter_three");
$totalchapter_threeSubmitted = $stmt->fetchColumn();


$stmt = $pdo->query("SELECT COUNT(*) FROM chapter_three WHERE status = 'pending'");
$totalchapter_threeSubmittedPending = $stmt->fetchColumn();


$stmt = $pdo->query("SELECT COUNT(*) FROM chapter_three WHERE status = 'approved'");;
$totalchapter_threeSubmittedApproved = $stmt->fetchColumn();


$stmt = $pdo->query("SELECT COUNT(*) FROM chapter_three WHERE status = 'rejected'");;
$totalchapter_threeSubmittedRejected = $stmt->fetchColumn();


// thesis chapter four query in the dashboard
$stmt = $pdo->query("SELECT COUNT(*) FROM chapter_four");
$totalchapter_fourSubmitted = $stmt->fetchColumn();


$stmt = $pdo->query("SELECT COUNT(*) FROM chapter_four WHERE status = 'pending'");
$totalchapter_fourSubmittedPending = $stmt->fetchColumn();


$stmt = $pdo->query("SELECT COUNT(*) FROM chapter_four WHERE status = 'approved'");;
$totalchapter_fourSubmittedApproved = $stmt->fetchColumn();


$stmt = $pdo->query("SELECT COUNT(*) FROM chapter_four WHERE status = 'rejected'");;
$totalchapter_fourSubmittedRejected = $stmt->fetchColumn();



// thesis chapter five query in the dashboard
$stmt = $pdo->query("SELECT COUNT(*) FROM chapter_five");
$totalchapter_fiveSubmitted = $stmt->fetchColumn();


$stmt = $pdo->query("SELECT COUNT(*) FROM chapter_five WHERE status = 'pending'");
$totalchapter_fiveSubmittedPending = $stmt->fetchColumn();


$stmt = $pdo->query("SELECT COUNT(*) FROM chapter_five WHERE status = 'approved'");;
$totalchapter_fiveSubmittedApproved = $stmt->fetchColumn();


$stmt = $pdo->query("SELECT COUNT(*) FROM chapter_five WHERE status = 'rejected'");;
        ...
        <div class="sidebar-wrapper sidebar-theme">
            ...
            <nav id="sidebar">
                ...
                <ul class="list-unstyled menu-categories" id="accordionExample">
                    ...
                    <li class="menu">
                        <a href="./generate_admin_report.php"  target="_blank"  aria-expanded="false" class="dropdown-toggle">
                            <div class="">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-user-check">
                                <path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path>
                                <circle cx="8.5" cy="7" r="4"></circle>
                        ...
                        <div class="col-xl-3 col-lg-6 col-md-6 col-sm-6 layout-spacing">
                            <div class="widget widget-t-sales-widget widget-m-sales">
                                <div class="media">
                                    ...
                                    <div class="media-body">
                                    <p class="widget-text">Number Of Faculties: </p>
                                        <p class="widget-numeric-value"><?php echo $totalFaculties; ?></p>
                                    </div>
                                </div>
