                ...
                <div class="row layout-spacing layout-top-spacing" id="cancel-row">
                    <div class="col-lg-12">
                        <div class="container py-5">
                            <h2 class="text-center mb-5"><i class="fas fa-file-alt"></i> Thesis Chapter Five Review</h2>
                            
                            <?php if (!empty($chapter_five)): ?>
                                <?php foreach ($chapter_five as $chapter): ?>
                                    <div class="card mb-4 proposal-card">
                                        <div class="card-header bg-primary text-white">
                                            <h1 class="card-title h5 text-white mb-0 text-center"><?php echo htmlspecialchars($chapter['title']); ?></h1>
                                        </div>
                                        <div class="card-body">
                                            ...
                                            <?php if (!empty($chapter['file_path'])): ?>
                                                <div class="card mt-4">
                                                    <div class="card-header bg-primary text-white">
                                                        <h5 class="mb-0 text-white"><i class="fas fa-file-pdf"></i> Chapter Five Document</h5>
                                                    </div>
                                                    <div class="card-body">
                                                        <div class="d-flex justify-content-between align-items-center">
                                                            <span>Access the Chapter Five document:</span>
                                                            <div>
                                            ...
                                            <button type="button" class="btn btn-primary mt-3" data-bs-toggle="modal" data-bs-target="#reviewModal<?php echo $chapter['id']; ?>">
                                                <i class="fas fa-edit"></i> Review Chapter Five
                                            </button>
                                        </div>
                                    </div>
                                        ...
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="reviewModalLabel<?php echo $chapter['id']; ?>">Review Chapter Five</h5>
                            ...
                            <?php else: ?>
                                <div class="alert alert-info" role="alert">
                                    No Chapter Five Submission found for this student.
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $update_query = "UPDATE chapter_five SET status = ?, comment = ?, lecturer_comment_time = CURRENT_TIMESTAMP WHERE id = ?";
        $update_stmt = $pdo->prepare($update_query);
        $update_stmt->execute([$status, $comment, $chapter_id]);


        // Insert into chapter_interactions table
        $insert_query = "INSERT INTO chapter_five_interactions (chapter_five_id, user_id, title, description, message) VALUES (?, ?, ?, ?, ?)";
        $insert_stmt = $pdo->prepare($insert_query);
        $insert_stmt->execute([$chapter_id, $supervisor_id, "Chapter Five  Review", "Status: $status", $comment]);
        ...
        echo "<script>
            ...
            setTimeout(function() {
                window.location.href = 'view_student_chapter5.php?student_id=" . $student_id . "';
            }, 1000);
        </script>";
